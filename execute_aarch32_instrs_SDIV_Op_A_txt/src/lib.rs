...
use common::*;
pub fn execute_aarch32_instrs_SDIV_Op_A_txt<T: Tracer>(
    state: &mut State,
    tracer: &T,
    d: i64,
    m: i64,
    ...
    fn block_1<T: Tracer>(
        ...
        // D s_1_4: cast sx s_1_3 -> i
        let s_1_4: i128 = {
            let sign_bit = s_1_3.length() - 1;
            let mut result = s_1_3.value() as i128;
            if ((result >> sign_bit) & 1) == 1 {
                let cleared_bit = result & !(1 << sign_bit);
                result = cleared_bit - (1 << sign_bit);
            }
        ...
        // D s_1_13: cast sx s_1_12 -> i
        let s_1_13: i128 = {
            let sign_bit = s_1_12.length() - 1;
            let mut result = s_1_12.value() as i128;
            if ((result >> sign_bit) & 1) == 1 {
                let cleared_bit = result & !(1 << sign_bit);
                result = cleared_bit - (1 << sign_bit);
            }
